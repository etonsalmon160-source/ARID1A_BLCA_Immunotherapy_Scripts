...
attributes <- c("FRACTION_GENOME_ALTERED", "ANEUPLOIDY_SCORE")
...
# Drop rows where FRACTION_GENOME_ALTERED is NA
samples_df <- samples_df[!is.na(samples_df$FRACTION_GENOME_ALTERED), ]
message("Total samples with FGA data: ", nrow(samples_df))
...
fetch_body <- list(
  ...
  sampleIds = samples_df$SAMPLE_ID
)
...
samples_df$ARID1A_Status <- ifelse(samples_df$SAMPLE_ID %in% mutated_samples, "Mutant", "Wild-Type")

# 3. Perform statistical analysis
message("\n=== Comparisons in R ===")

wt_fga <- na.omit(samples_df[samples_df$ARID1A_Status == "Wild-Type", "FRACTION_GENOME_ALTERED"])
mut_fga <- na.omit(samples_df[samples_df$ARID1A_Status == "Mutant", "FRACTION_GENOME_ALTERED"])
...
wt_aneu <- na.omit(samples_df[samples_df$ARID1A_Status == "Wild-Type", "ANEUPLOIDY_SCORE"])
mut_aneu <- na.omit(samples_df[samples_df$ARID1A_Status == "Mutant", "ANEUPLOIDY_SCORE"])
...
message("\nGenerating verification plot in R...")
p1 <- ggplot(samples_df, aes(x = ARID1A_Status, y = FRACTION_GENOME_ALTERED, fill = ARID1A_Status)) +
  geom_violin(alpha = 0.6, trim = FALSE, show.legend = FALSE) +
  geom_boxplot(width = 0.1, fill = "white", outlier.shape = NA, show.legend = FALSE) +
  geom_jitter(width = 0.2, alpha = 0.3, size = 1, show.legend = FALSE) +
  scale_fill_manual(values = c("Wild-Type" = "#4DBBD5", "Mutant" = "#E64B35")) +
  theme_classic() +
  labs(title = "Fraction Genome Altered", x = "ARID1A Status", y = "FGA") +
...
p2 <- ggplot(samples_df, aes(x = ARID1A_Status, y = ANEUPLOIDY_SCORE, fill = ARID1A_Status)) +
  geom_violin(alpha = 0.6, trim = FALSE, show.legend = FALSE) +
  geom_boxplot(width = 0.1, fill = "white", outlier.shape = NA, show.legend = FALSE) +
  geom_jitter(width = 0.2, alpha = 0.3, size = 1, show.legend = FALSE) +
  scale_fill_manual(values = c("Wild-Type" = "#4DBBD5", "Mutant" = "#E64B35")) +
  theme_classic() +
  labs(title = "Aneuploidy Score", x = "ARID1A Status", y = "Score") +
  coord_cartesian(ylim = c(-5, 45)) +
  annotate("text", x = 1.5, y = 41, label = sprintf("p = %.4f", pval_aneu), size = 4.5, fontface = "bold", color = "#2C3E50")

# Save as PNG using R's built-in grid package (no extra dependencies required)
png("bladder_tcga_bulk_cnv_r_verification.png", width = 2400, height = 1200, res = 300)
library(grid)
grid.newpage()
...
dev.off()
message("R verification plot saved to bladder_tcga_bulk_cnv_r_verification.png")
