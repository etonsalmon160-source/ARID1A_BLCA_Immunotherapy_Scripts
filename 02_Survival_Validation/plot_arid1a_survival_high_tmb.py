# plot_arid1a_survival_high_tmb.py
...
# Fetch ARID1A (Entrez ID: 8289) mutations
...
fetch_data = {"entrezGeneIds": [8289], "sampleIds": sub_sample_ids}
...
sub_df['ARID1A_Status'] = sub_df['SAMPLE_ID'].apply(lambda x: 'Mutant' if x in mutated_samples else 'Wild-Type')

mut_df = sub_df[sub_df['ARID1A_Status'] == 'Mutant']
wt_df = sub_df[sub_df['ARID1A_Status'] == 'Wild-Type']
...
kmf_wt.fit(wt_df['OS_MONTHS'], event_observed=wt_df['EVENT'], label=f'ARID1A Wild-Type (N={len(wt_df)}, mOS={wt_df["OS_MONTHS"].median():.1f} mo)')
kmf_mut.fit(mut_df['OS_MONTHS'], event_observed=mut_df['EVENT'], label=f'ARID1A Mutant (N={len(mut_df)}, mOS={mut_df["OS_MONTHS"].median():.1f} mo)')
...
plt.title('Overall Survival in High-TMB Bladder Cancer (TMB >= 11.7)\nby ARID1A Mutation (Pure PD-1/PD-L1 Monotherapy Cohort)', fontsize=12, fontweight='bold', pad=15)
...
cph = CoxPHFitter()
sub_df['Is_Mutant'] = sub_df['ARID1A_Status'].apply(lambda x: 1 if x == 'Mutant' else 0)
