# plot_tert_survival_high_tmb.py
...
# Fetch TERT (Entrez ID: 7015) mutations
print("2. Fetching TERT mutations...")
url_mut = f'https://www.cbioportal.org/api/molecular-profiles/{study_id}_mutations/mutations/fetch'
fetch_data = {"entrezGeneIds": [7015], "sampleIds": sub_sample_ids}
...
sub_df['TERT_Status'] = sub_df['SAMPLE_ID'].apply(lambda x: 'Mutant' if x in mutated_samples else 'Wild-Type')

mut_df = sub_df[sub_df['TERT_Status'] == 'Mutant']
wt_df = sub_df[sub_df['TERT_Status'] == 'Wild-Type']
...
kmf_wt.fit(wt_df['OS_MONTHS'], event_observed=wt_df['EVENT'], label=f'TERT Wild-Type (N={len(wt_df)}, mOS={wt_df["OS_MONTHS"].median():.1f} mo)')
kmf_mut.fit(mut_df['OS_MONTHS'], event_observed=mut_df['EVENT'], label=f'TERT Mutant (N={len(mut_df)}, mOS={mut_df["OS_MONTHS"].median():.1f} mo)')
...
plt.title('Overall Survival in High-TMB Bladder Cancer (TMB >= 11.7)\nby TERT Mutation (Pure PD-1/PD-L1 Monotherapy Cohort)', fontsize=12, fontweight='bold', pad=15)
...
cph = CoxPHFitter()
sub_df['Is_Mutant'] = sub_df['TERT_Status'].apply(lambda x: 1 if x == 'Mutant' else 0)
...
output_path = 'bladder_high_tmb_tert_km_plot.png'
plt.tight_layout()
