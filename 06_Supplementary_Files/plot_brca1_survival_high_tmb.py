# plot_brca1_survival_high_tmb.py
...
# High TMB cohort (80th percentile of pure cohort: TMB >= 15.64)
tmb_thresh = 15.64
sub_df = blca_df[blca_df['TMB_NONSYNONYMOUS'] >= tmb_thresh].copy()
sub_sample_ids = sub_df['SAMPLE_ID'].tolist()

print(f"Pure immunotherapy High TMB cohort size (>= 15.64): {len(sub_df)}")

# Fetch BRCA1 (Entrez ID: 672) mutations
print("2. Fetching BRCA1 mutations...")
url_mut = f'https://www.cbioportal.org/api/molecular-profiles/{study_id}_mutations/mutations/fetch'
fetch_data = {"entrezGeneIds": [672], "sampleIds": sub_sample_ids}
...
sub_df['BRCA1_Status'] = sub_df['SAMPLE_ID'].apply(lambda x: 'Mutant' if x in mutated_samples else 'Wild-Type')

mut_df = sub_df[sub_df['BRCA1_Status'] == 'Mutant']
wt_df = sub_df[sub_df['BRCA1_Status'] == 'Wild-Type']
...
kmf_wt.fit(wt_df['OS_MONTHS'], event_observed=wt_df['EVENT'], label=f'BRCA1 Wild-Type (N={len(wt_df)}, mOS={wt_df["OS_MONTHS"].median():.1f} mo)')
kmf_mut.fit(mut_df['OS_MONTHS'], event_observed=mut_df['EVENT'], label=f'BRCA1 Mutant (N={len(mut_df)}, mOS={mut_df["OS_MONTHS"].median():.1f} mo)')
...
plt.title('Overall Survival in High-TMB Bladder Cancer (TMB >= 15.6)\nby BRCA1 Mutation (Pure PD-1/PD-L1 Monotherapy Cohort)', fontsize=12, fontweight='bold', pad=15)
...
cph = CoxPHFitter()
sub_df['Is_Mutant'] = sub_df['BRCA1_Status'].apply(lambda x: 1 if x == 'Mutant' else 0)
...
output_path = 'bladder_high_tmb_brca1_km_plot.png'
plt.tight_layout()
